...
const logger$3 = LogManager.get().getLogger(), OfflineDatabaseObjectListProperty = asProperty((config, store, widgetId, propertyPath) => {
  ...
  return getServerPagedList$(config, config.sort ? convertToSortInstructions(config.sort, config.dataSourceId) : [], store, widgetId, fetchParameters$, async function(parameter, { offset, limit, sortOrder, filter }, needTotalCount) {
    ...
    logger$3.debug(`Fetching data for entity '${config.entity}' for widget ${toFriendlyId(widgetId)}`);
    const { mxObjects, count } = await offlineData().retrieve(config.entity, combinedFilter, options);
    return function(objects) {
      logger$3.debug(`Received ${objects.length} objects for widget ${toFriendlyId(widgetId)}`, "object ids:", objects.map((mxobj) => mxobj.getGuid()));
    }(mxObjects), { mxObjects, totalCount: needTotalCount ? count : void 0, hasMoreItems: offset + limit < count };
  });
});

const logger$2 = LogManager.get().getLogger(), OnlineDatabaseObjectListProperty = asProperty((config, store, widgetId, propertyPath) => getServerPagedList$(config, config.sort ? convertToSortInstructions(config.sort, config.dataSourceId) : [], store, widgetId, function(fetchOnlyWithAllParams = false) {
  ...
  !function(runtimeArgs) {
    ...
    logger$2.debug(Object.values(runtimeArgs).length ? `Fetching data using XPath with arguments '${JSON.stringify(runtimeArgs)}' and operationId '${config.operationId}' for widget ${friendlyId}` : `Fetching data using XPath with operationId '${config.operationId}' for widget ${friendlyId}`);
  }(args);
...
function listItemProperty(getValue$, description, dataSourceId, itemValueTemplate, legacyItems = false) {
  ...
  return [fromComputation$(`Value for ${description}`, () => {
    ...
    return Object.assign(legacyItems ? (item) => (logger$1.warn(`DEPRECATED: direct usage of ${description} as a function, use .get() instead -- will be removed in version: 10.0`), itemGetter(item)) : {}, itemValueTemplate, { get: itemGetter });
  }), function() {
...
export { DatabaseObjectListProperty, ListAttributeProperty, listItemProperty, withFirstLoadDoneSharing$ };
