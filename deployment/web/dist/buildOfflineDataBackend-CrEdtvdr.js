...
}), addInaccessibleColumnMigration(), async (lowLevelDatabaseAPI) => {
  await lowLevelDatabaseAPI.inTransaction(async (tx) => {
    await tx.selectValue(`SELECT (${createTableCountLikeNameQuery("$" + DB_META_TABLE)}) >= (${createTableCountLikeNameQuery("$" + METADATA_TABLE)})`, []) && await tx.exec(`DROP TABLE IF EXISTS ${DB_META_TABLE}`, []);
  }), await addInaccessibleColumnMigration()(lowLevelDatabaseAPI);
}], Migrations = MigrationFunctions.map((m, index) => ({ version: index + 1, apply: m }));
function addInaccessibleColumnMigration() {
  return async (lowLevelDatabaseAPI) => {
    const metadataTable = toUserScopedName(METADATA_TABLE);
    if (await async function(tx, tableName, columnName) {
      return (await tx.selectObjects(`SELECT * FROM pragma_table_info('${tableName}') WHERE name='${columnName}'`)).length > 0;
    }(lowLevelDatabaseAPI, metadataTable, INACCESSIBLE_COLUMN))
      return;
    const [addColumnQuery, addColumnParams] = createAddColumnSqlQueries(metadataTable, { name: INACCESSIBLE_COLUMN, type: "text" });
    return lowLevelDatabaseAPI.inTransaction(async (tx) => {
      await tx.exec(addColumnQuery, addColumnParams), await tx.exec(`UPDATE ${metadataTable} SET ${INACCESSIBLE_COLUMN} = '[]'`);
    });
  };
}
function createTableCountLikeNameQuery(name) {
  return `SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name LIKE '%${name}'`;
}
...
async function executeOfflineMicroflow(operationId, mfArgs, schema, objectCache, lockManager, database, fileBackend, targetForm, validationGuids = [], runAsync = false, logger) {
  let runtimeResponse;
  try {
    ...
    }(getRequestDataForMicroflow(objectCache, operationId, getGuidsFromRuntimeArguments(mfArgs).concat(getImplicitMicroflowParameters()))), filteredChanges = function(objectCache2, changes2) {
    ...
    }(objectCache, changes), filteredObjects = function(objectCache2, objects2) {
      return objects2.map((obj) => {
        const entity = obj.objectType, filteredAttributes = Object.fromEntries(Object.entries(obj.attributes).filter(([attr, value]) => !isAssociationToUnsyncedOffline(entity, attr, value.value)));
        return { ...obj, attributes: filteredAttributes };
      });
    }(0, objects);
  ...
  await async function(runtimeResponse2, fileBackend2, database2, objectCache2, schema2, logger2) {
    ...
    if (0 !== runtimeGuidsToPush.length) {
      const guidsFailedToDownload = await downloadFiles(downloads, fileBackend2, logger2), objectsWithoutDownloadErrors = 0 === guidsFailedToDownload.size ? objectsToPush : objectsToPush.filter((obj) => !guidsFailedToDownload.has(obj.guid));
      await handleOfflineResponseObjects(runtimeGuidsToPush, database2, objectsWithoutDownloadErrors, objectCache2, schema2, syncedObjsRuntimeToOfflineMap, new RuntimeToOfflineGuidMapping(), 0 !== objectsToPush.length ? "clean" : "nothing");
      const filesToMove = guidsFailedToDownload.size > 0 ? moves.filter(([_source, _dest, guid]) => !guidsFailedToDownload.has(guid)) : moves;
      await executeFileInstructions({ moves: filesToMove }, fileBackend2), executeFileInstructions({ deletes }, fileBackend2).catch(handleError);
    }
  }(runtimeResponse, fileBackend, database, objectCache, schema, logger);
    ...
    return await async function(database2, fileBackend2, cache2, { schema, preserveData, fetch, never, online }, uploadedObjsEntityToGuidsMap2, resetPreservedEntities2, incompatibleEntities2, logger2) {
      ...
      return logger2.trace(`Computing subscription updates completed in ${stopwatch.measure()} milliseconds.`), logger2.trace("Rebuilding the database."), stopwatch.reset(), await async function(database3, schema2, objs, entityToGuidsMapToDelete, entitiesToPreserve, excludedEntities, incompatibleEntities3 = /* @__PURE__ */ new Set()) {
        ...
        await database3.usingWriteTransaction(async (tx) => {
          ...
          await tx.dropEntityTables(entitiesToDrop), fullReset ? (await tx.dropSystemTables(), await tx.updateEntityMetadatas(getSession().getOfflineMetadata())) : await tx.deleteEntitiesByGuid(Object.values(entityToGuidsMapToDelete).flat()), await tx.ensureAppSchema(entitiesToReset), await tx.insertMxObjects(objs);
        });
...
class OfflineData {
  ...
  async initialize(migrations, webSqlConnection) {
    ...
    const latestVersion = function(migrations2) {
      return migrations2[migrations2.length - 1]?.version ?? 0;
    }(migrations), dbVersion = await this.database.usingWriteTransaction(async (tx) => (await tx.ensureSystemSchema(latestVersion), tx.getDatabaseVersion()));
  ...
  async executeMicroflow(operationId, args, targetForm, validationGuids = [], runAsync = false) {
    return executeOfflineMicroflow(operationId, args, this.syncConfig.schema, this.objectCache, this.lockManager, this.database, this.fileBackend, targetForm, validationGuids, runAsync, this.syncLogger);
  }
